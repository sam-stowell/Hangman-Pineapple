...
class WordGuessingGame(tk.Tk):
    ...
    def show_start_menu(self):
        ...
        # # Add image file
        # bg = tk.PhotoImage(file="menubackground.png")
        #
        # # Create Canvas
        # canvas1 = tk.Canvas(self.current_frame, width=400,
        #                     height=400)
        #
    ...
    def choose_word(self, category):
        ...
        for char in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":  # Iterate through the alphabet
            button = tk.Button(alphabet_frame, text=char, font=BUTTON_FONT,
                               ...
                               activebackground="grey",  # Change color when the button is clicked
    ...
    def handle_alphabet_button(self, char, word_label, tries_label, incorrect_guess_label, congrats_label):
        ...
        # Find all buttons with matching text (assuming unique characters)
        matching_buttons = [button for button in self.winfo_children() if
                            isinstance(button, tk.Button) and button.cget('text') == char]

        # Assuming there's only one button with the clicked character
        if matching_buttons:
            clicked_button = matching_buttons[0]
